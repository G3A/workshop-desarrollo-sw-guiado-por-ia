...
- **Una versión por liberación.** `main` es lo publicado; `dev` es integración. Cada PR a `dev`
  ...
  **PR de liberación `dev` → `main`** sube `version` en `sdlc-ia/.claude-plugin/plugin.json`,
  convierte esa sección en la versión nueva y le pone la fecha del merge a `main`. La versión es lo

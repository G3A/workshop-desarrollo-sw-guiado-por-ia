...
The ledger follows the same output language resolved in `SKILL.md` (Spanish by default, English
with `en`):
...
Generated by the `agent-context-java` skill. Tracks the documentation's key factual claims, their
source in the repository, and whether a person confirmed them. Re-run the skill to update it.

| Claim | Source | Confidence | Status |
|---|---|---|---|
| The project targets JDK 21. | `pom.xml:24` | high | confirmed |
| Persistence is JPA/Hibernate. | inferred | medium | corrected → Spring Data JDBC |
| Flyway migrations run at startup. | user | low | TODO: verify |
```

Status values in Spanish: `confirmada` / `corregida → …` / `TODO: verificar`. In English:
`confirmed` / `corrected → <new value>` / `TODO: verify`.

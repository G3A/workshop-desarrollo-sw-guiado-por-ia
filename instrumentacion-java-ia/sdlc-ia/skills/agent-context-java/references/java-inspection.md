...
The discovery work behind `docs/java.md` — what to read, extract, and record, per section. Read
real files (Glob/Grep/Read); where a fact isn't readable, leave `<!-- TODO -->` — never guess.

**Every section is conditional.** Inspect only what the repo signals, and carry that into the doc:
a single-module Spring Boot app gets a `docs/java.md` with the multi-module/Modulith sections
**deleted**, not filled with TODOs. A short accurate doc beats a long hedged one.

## Adjacent signals

A Java repo is rarely only Java. These feed `architecture.md`/`infrastructure.md`:

| Signal | Infer |
|---|---|
| `Dockerfile`, `docker-compose*.yml`, `compose.yaml` | Containerization (packaging — §12, `java-inspection-2.md`) |
...
| `Makefile` | Dev shortcuts over Maven/Gradle/Docker — prefer these in AGENTS.md |
...
- **Record:** a module table (name · type · one-line purpose) and, for multi-module repos, a small
  Mermaid graph. Single-module + Spring Modulith repos skip this table — §6 covers it instead.
...
- **Extract:** the JDK release the build targets, and whether it differs from what's installed in
  CI/Docker (e.g. `<java.version>21</java.version>` but a Dockerfile `FROM eclipse-temurin:17`).
- **Record:** the JDK target; flag a declared-vs-used mismatch as an ambiguity for claim
  validation.
...
- **Extract:** where beans are wired (a few central `@Configuration` classes vs scattered
  ...
  injection is a smell), and conditional wiring (`@Profile`, `@ConditionalOnProperty`).
...
- **Extract:** the everyday commands — build, run (`spring-boot:run`/`bootRun`), test, migrate; a
  ...
  HTTP stubs (WireMock).
- **Record:** the 3–6 commands a developer runs, the runner/framework(s), and how tests
  are organized.
...
- **Extract:** which gates are **present** vs **absent**. Only-ArchUnit, no
  Checkstyle/Spotless/SpotBugs/PMD, is a real common config — record it as-is ("ArchUnit present,
  nothing else"), don't imply more coverage than exists.
...
- **Record:** the API tech and any wiring an agent must respect. Omit for headless services (batch
  jobs, CLIs, libraries).

**§12–§14 continue in `references/java-inspection-2.md`** (deployment/packaging, cross-cutting
concerns, gotchas/hotspots).

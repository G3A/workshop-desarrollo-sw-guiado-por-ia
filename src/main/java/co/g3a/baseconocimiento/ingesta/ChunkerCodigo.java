...
/**
 * Chunker heurístico para código fuente: clase → método → bloque, de grano
 * grueso a fino, subdividiendo SOLO cuando el fragmento excede el tamaño
 * máximo — igual que {@link ChunkerEncabezados} hace con las secciones de
 * Markdown, pero detectando límites de clase/método por regex sobre líneas en
 * vez de niveles de encabezado.
 *
 * <p>No es un parser: no hay binding sólido de tree-sitter en Java (ver el ADR
 * correspondiente). Funciona razonablemente en lenguajes de llave (Java, C#,
 * JS/TS, Go, C/C++); en lenguajes sin llaves como Python no encuentra
 * declaraciones y cae directo a {@link ChunkerVentanas} sobre el archivo
 * entero — limitación heurística documentada, no una decisión de diseño.
 */
final class ChunkerCodigo {
    ...
    private static final Pattern DECLARACION_TIPO = Pattern.compile(
            "^\\s*(public|private|protected|internal|static|final|abstract|sealed|export)?\\s*"
                    + "(class|interface|enum|record|struct)\\s+\\w+.*$");

    private static final Pattern DECLARACION_METODO = Pattern.compile(
    ...
    /** @param ruta declaración de clase (y de método, si aplica) que contextualiza este bloque; {@code null} si no se detectó ninguna */
    ...
    private static List<Integer> indicesQueCoincidenCon(List<String> lineas, Pattern patron, boolean excluirControl) {

...
/**
 * Cliente HTTP hacia Microsoft Graph: token de aplicación por
 * {@code client_credentials} (cacheado hasta cerca de su vencimiento, mismo
 * patrón que {@code ClienteConectorBotFramework} del adaptador de Teams) y un
...
class ClienteGraph {
    ...
    /**
     * @param urlAbsolutaOPath un path relativo a {@code graph-base-url}, o una URL
     *                         absoluta como un {@code @odata.nextLink}/{@code deltaLink}
     */
    String get(String urlAbsolutaOPath) {
        String url = urlAbsolutaOPath.startsWith("http")
                ? urlAbsolutaOPath
    ...
    private TokenCacheado pedirToken() {
        ...
        String respuesta = restClient.post().uri(url)
        ...
        return new TokenCacheado(
                nodo.get("access_token").asString(), Instant.now().plusSeconds(nodo.get("expires_in").asLong()));
    }
}

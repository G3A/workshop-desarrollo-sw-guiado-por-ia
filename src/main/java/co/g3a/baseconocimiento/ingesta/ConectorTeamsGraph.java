...
/**
 * Ingiere un canal de Teams vía Microsoft Graph: delta query de mensajes
 * raíz, sus respuestas, destilación del hilo completo por LLM y bursting de
 * los mensajes individuales que superan el gate de IDF/longitud.
 *
 * <p>A diferencia de {@link ConectorDocumentosLocales} y
 * {@link ConectorReposLocales}, la destilación aquí SÍ pasa por el LLM — ver
 * {@code package-info} del módulo: un hilo de chat es contenido conversacional
 * ruidoso, no un documento ya estructurado.
 */
@Component
class ConectorTeamsGraph {
    ...
    ConectorTeamsGraph(IngestaRepositorio repo, ClienteGraph cliente, Destilador destilador, GraphPropiedades propiedades) {
        this.repo = repo;
    ...
    Resumen ingerir() {
        ...
        long sourceId = repo.obtenerOCrearFuente(KIND, propiedades.teamId() + "/" + propiedades.channelId());
        String url = repo.obtenerSyncState(sourceId)
                .map(Json::leer)
                .map(nodo -> nodo.get("delta_link"))
                .filter(n -> n != null && !n.isNull())
                .map(JsonNode::asString)
                .orElse("/teams/" + propiedades.teamId() + "/channels/" + propiedades.channelId() + "/messages/delta");
        ...
        while (url != null) {
            ...
            for (JsonNode mensaje : pagina.get("value")) {
                ...
                if (esBorrado(mensaje)) {
                    repo.buscarDocumento(sourceId, messageId)
                            .ifPresent(d -> repo.eliminarDocumento(d.id()));
                    continue;
                }
                hilosVistos++;
        ...
        log.info("Ingesta de Teams por Graph: {} hilos vistos, {} actualizados, {} chunks nuevos",
                hilosVistos, hilosActualizados, chunksCreados);
    ...
    private int ingerirHilo(long sourceId, JsonNode raiz, String messageId) {
        ...
        long documentoId = repo.upsertDocumento(sourceId, messageId,
                "https://teams.microsoft.com/l/message/" + propiedades.channelId() + "/" + messageId,
                "Hilo de Teams", textoCompleto, hash, ProyectoId.POR_DEFECTO.valor());

        Destilador.Destilado destilado = destilador.destilar(textoCompleto);
        String distilledHilo = Json.escribir(Map.of(
        ...
        int ord = 0;
        long chunkHilo = repo.insertarChunk(
                documentoId, sourceId, ProyectoId.POR_DEFECTO.valor(), ord++, "thread", textoCompleto, distilledHilo);
        ...
        for (JsonNode r : respuestas) {
            ...
            long chunkBurst = repo.insertarChunk(
                    documentoId, sourceId, ProyectoId.POR_DEFECTO.valor(), ord++, "thread_burst", textoBurst, distilledBurst);
    ...
    private List<JsonNode> obtenerRespuestas(String messageId) {
        ...
        String url = "/teams/" + propiedades.teamId() + "/channels/" + propiedades.channelId()

 ...
 *
 * <p>Ambas fuentes son contenido ya estructurado (título/descripción de un
 * work item, encabezados de una página de wiki) — la destilación es
 * heurística, igual que {@link ConectorDocumentosLocales}, sin pasar por el
 * LLM. Eso es propio de {@code ConectorTeamsGraph} (hilos de chat).
 *
 * <p>Incrementalidad por hash de contenido, igual que los demás conectores:
 * se vuelve a consultar todo en cada corrida (WIQL no tiene un filtro de
 * "cambiado desde" tan simple como el delta link de Graph), pero solo lo que
 * cambió se re-trocea y se vuelve a encolar para embeber.
 */
@Component
class ConectorAzureDevOps {
    ...
    private static final String WIQL_QUERY = "SELECT [System.Id] FROM WorkItems ORDER BY [System.ChangedDate] DESC";
    ...
    ConectorAzureDevOps(IngestaRepositorio repo, ClienteAzureDevOps cliente, AzureDevOpsPropiedades propiedades) {
        this.repo = repo;
    ...
    record Resumen(
            int workItemsVistos, int workItemsActualizados,
            int paginasWikiVistas, int paginasWikiActualizadas, int chunksCreados) {
    }
    ...
    Resumen ingerir() {
        ...
        log.info("Ingesta de Azure DevOps: {} work items ({} actualizados), {} paginas de wiki ({} actualizadas)",
                workItems.vistos(), workItems.actualizados(), wiki.vistos(), wiki.actualizados());
        return new Resumen(
                workItems.vistos(), workItems.actualizados(),
                wiki.vistos(), wiki.actualizados(), workItems.chunks() + wiki.chunks());
    }

    private ResultadoParcial ingerirWorkItems() {
        long sourceId = repo.obtenerOCrearFuente(KIND, "work_items");

        String urlWiql = "/" + propiedades.org() + "/" + propiedades.proyecto() + "/_apis/wit/wiql?api-version=7.1";
        JsonNode resultadoWiql = Json.leer(cliente.post(urlWiql, Json.escribir(Map.of("query", WIQL_QUERY))));
        ...
        Map<String, Object> cuerpoLote = Map.of(
                "ids", ids,
                "fields", List.of("System.Id", "System.Title", "System.Description", "System.WorkItemType"));
        JsonNode lote = Json.leer(cliente.post(
                "/" + propiedades.org() + "/_apis/wit/workitemsbatch?api-version=7.1", Json.escribir(cuerpoLote)));
        ...
        if (valores != null) {
            for (JsonNode wi : valores) {
                ...
                String descripcion = campos == null ? "" : stripHtml(Json.textoDe(campos, "System.Description"));
                String texto = (titulo + "\n" + descripcion).strip();
                ...
                String uri = "https://dev.azure.com/" + propiedades.org() + "/" + propiedades.proyecto()
                        + "/_workitems/edit/" + id;
    ...
    private ResultadoParcial ingerirWiki() {
        ...
        for (String ruta : rutas) {
            ...
            String uri = "https://dev.azure.com/" + propiedades.org() + "/" + propiedades.proyecto()
                    + "/_wiki/wikis/" + propiedades.wiki() + "?pagePath=" + ruta;
            long documentoId =
                    repo.upsertDocumento(sourceId, ruta, uri, ruta, contenido, hash, ProyectoId.POR_DEFECTO.valor());

            int ord = 0;
            for (ChunkerEncabezados.Seccion seccion : ChunkerEncabezados.trocear(contenido)) {
                String resumen = resumenDe(seccion.cuerpo());
                String tituloSeccion = seccion.rutaEncabezados().isEmpty()
                        ? resumen
                        : String.join(" › ", seccion.rutaEncabezados()) + ": " + resumen;
                String distilled = Json.escribir(Map.of("summary", tituloSeccion));
                long chunkId = repo.insertarChunk(documentoId, sourceId, ProyectoId.POR_DEFECTO.valor(), ord++,
                        "wiki_section", seccion.cuerpo(), distilled);
                repo.encolarEmbeberChunk(chunkId);
                chunksCreados++;
            }
            actualizados++;
    ...
    private String urlPagina(String path, boolean recursionFull, boolean includeContent) {
        UriComponentsBuilder builder = UriComponentsBuilder
                .fromPath("/{org}/{proyecto}/_apis/wiki/wikis/{wiki}/pages")
        ...
        return builder.buildAndExpand(propiedades.org(), propiedades.proyecto(), propiedades.wiki(), path)
                .encode().toUriString();
    }

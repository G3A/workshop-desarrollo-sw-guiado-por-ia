...
/**
 * El SQL propio de {@code recent_commits} y {@code subsystem_index}: no son
 * una señal más de {@code recuperacion} (no rankean por relevancia a una
 * consulta), son listados/agregados directos sobre el corpus.
 */
@Repository
class HerramientasRepositorio {
    ...
    /**
     * {@code recent_commits}: el primer chunk de cada documento de una fuente,
     * por frescura. Hoy solo hay fuentes {@code local_docs}; devuelve vacío para
     * {@code local_git} hasta que el conector de F6 exista — no es un error, es
     * el estado esperado de un conector todavía deshabilitado.
     */
    List<Fragmento> masRecientesPorFuente(String sourceKind, String projectId, int limite) {
        return jdbc.sql("""
                        ...
                        LIMIT :limite
                        """)
                .param("sourceKind", sourceKind).param("projectId", projectId).param("limite", limite)
                .query(HerramientasRepositorio::mapearFragmento).list();
    }

    /**
     * {@code subsystem_index}: agrega {@code distilled.systems_mentioned} across
     * el corpus. Ese campo solo lo puebla la destilación por LLM de hilos de
     * Teams (F6); para documentos y código de hoy, que son heurísticos (ver
     * {@code package-info} de {@code ingesta}), queda vacío — el índice se
    ...
    List<Fragmento> indiceDeSubsistemas(String projectId, int limite) {
        return jdbc.sql("""
                        ...
                        LIMIT :limite
                        """)
                .param("projectId", projectId).param("limite", limite)
                .query(HerramientasRepositorio::mapearSubsistema).list();
    }

    /**
     * Cuantos chunks tiene el proyecto ahora mismo: la base del umbral dinamico
     * de {@link UmbralRelevancia} (ADR-0008) — no es una señal de relevancia,
     * es el tamaño del corpus contra el que se está preguntando.
     */
    ...
    private static Fragmento mapearFragmento(ResultSet rs, int n) throws SQLException {
        return new Fragmento(
                rs.getLong("id"), rs.getLong("document_id"), rs.getString("uri"), rs.getString("title"),
                rs.getString("text"), rs.getString("kind"), rs.getInt("ord"),
                rs.getTimestamp("source_updated_at").toInstant(), Map.of(), 0.0, null);
    }
    ...
    private static Fragmento mapearSubsistema(ResultSet rs, int n) throws SQLException {
        ...
        String texto = "Sistema \"%s\": mencionado en %d fragmentos del corpus. Ejemplo: %s"
                .formatted(sistema, menciones, rs.getString("ejemplo_titulo"));
        Instant actualizado = rs.getTimestamp("ejemplo_actualizado").toInstant();
        return new Fragmento(
                rs.getLong("ejemplo_id"), rs.getLong("ejemplo_documento_id"), rs.getString("ejemplo_uri"),
                "Subsistema: " + sistema, texto, "subsystem_summary", rs.getInt("ejemplo_ord"),
                actualizado, Map.of(), 0.0, null);
    }
}

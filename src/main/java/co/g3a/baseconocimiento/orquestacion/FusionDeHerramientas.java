...
/**
 * Etapa 4: combina los fragmentos de todas las herramientas que corrió el
 * {@link Executor} en una sola lista para la síntesis. Dos herramientas
 * distintas pueden traer el mismo chunk (p. ej. {@code search_unified} y
 * {@code search_docs} en la misma pregunta) — se deduplica por id, quedándose
 * con la instancia de mejor puntaje.
 *
 * <p>Pura y sin dependencias de Spring, igual que {@code RrfFusion} en
 * {@code recuperacion}: así se prueba por sus propios méritos sin levantar un
 * contexto.
 */
final class FusionDeHerramientas {
    ...
    static List<Fragmento> combinar(List<List<Fragmento>> resultadosPorHerramienta, int maxFragmentos) {

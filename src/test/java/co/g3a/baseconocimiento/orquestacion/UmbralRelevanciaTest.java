...
class UmbralRelevanciaTest {
    ...
    @Test
    @DisplayName("Umbral efectivo: interpola linealmente entre piso y techo segun chunks del proyecto")
    ...
    @Test
    @DisplayName("Ambiguo tambien atrapa el contraejemplo real: irrelevante que puntuo mas alto que lo relevante")
    void mandaAVerificacionElContraejemploDeAdr0008() {
        ...
        UmbralRelevancia.Resultado resultado = UmbralRelevancia.evaluar(List.of(irrelevantePeroConScoreAlto), 4, PROPS);

        assertThat(resultado.decision()).isEqualTo(Decision.AMBIGUO);
    }

    @Test
    @DisplayName("Ambiguo tambien atrapa el segundo contraejemplo: score alto sin ser una copia literal")
    void mandaAVerificacionElSegundoContraejemploDeAdr0008() {
        ...
        UmbralRelevancia.Resultado resultado = UmbralRelevancia.evaluar(List.of(scoreAltoSinCerteza), 4, PROPS);
    ...
    void aceptaSinVerificarUnaCopiaCasiLiteral() {
        ...
        UmbralRelevancia.Resultado resultado = UmbralRelevancia.evaluar(List.of(copiaLiteral), 4, PROPS);
    ...
    void rechazaSinFragmentos() {
        assertThat(UmbralRelevancia.evaluar(List.of(), 4, PROPS).decision()).isEqualTo(Decision.INSUFICIENTE);
    }
    ...
    void noBloqueaConFragmentosSinRerank() {
        Fragmento deListado = new Fragmento(1L, 1L, "file:///1", "t", "texto", "subsystem_summary",
                0, Instant.EPOCH, Map.of(), 0.0, null);
    ...
    private static Fragmento fragmento(double rerank) {
        return new Fragmento(1L, 1L, "file:///1", "t", "texto", "doc_section",
                0, Instant.EPOCH, Map.of(), 0.03, rerank);
    }
}

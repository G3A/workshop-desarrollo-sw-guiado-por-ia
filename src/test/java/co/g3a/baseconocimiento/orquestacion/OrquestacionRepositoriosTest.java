...
/**
 * El SQL propio de F3 contra un PostgreSQL real: los agregados de
 * {@code recent_commits} y {@code subsystem_index}, la expansion de contexto
 * por vecinos, y el registro en {@code query_log}.
 */
@SpringBootTest(
...
class OrquestacionRepositoriosTest {
    ...
    @Autowired
    QueryLogRepositorio queryLogRepo;
    ...
    void indiceDeSubsistemas() {
        ...
        assertThat(indice).extracting(Fragmento::titulo)
                .contains("Subsistema: auth", "Subsistema: billing");
        Fragmento auth = indice.stream().filter(f -> f.titulo().equals("Subsistema: auth")).findFirst().orElseThrow();
    ...
    void vecinosPorOrd() {
        ...
        assertThat(vecinos).extracting(ContextoRepositorio.Vecino::texto)
    ...
    void registraLaConsulta() {
        Fragmento fragmento = new Fragmento(1L, 1L, "file:///doc1", "Doc 1", "texto", "doc_section",
                0, Instant.EPOCH, Map.of(), 0.05, 8.0);
        var plan = new PlanDeHerramientas(List.of("search_unified"), "porque si");
        var ejecucion = new Executor.EjecucionHerramienta("search_unified", List.of(fragmento), 12L, null);
        var cita = new Cita("file:///doc1", "Doc 1", "texto", "doc_section");

        long id = queryLogRepo.registrar(
                "¿como se despliega?", "default", plan, List.of(ejecucion), List.of(fragmento),
                "Se despliega con docker compose. [1]", List.of(cita), 123L);

        assertThat(id).isPositive();

        String pregunta = jdbc.sql("SELECT question FROM query_log WHERE id = :id")
    ...
    private long crearFuente(String kind, String nombre) {
        return jdbc.sql("""
                        ...
                        """)
                .param("kind", kind).param("nombre", nombre)
    ...
    private long crearDocumento(long fuenteId, String externalId) {
        return jdbc.sql("""
                        ...
                        RETURNING id
                        """)
                .param("fuenteId", fuenteId).param("e", externalId)
                .query(Long.class).single();
    }

    private void insertarChunk(long fuenteId, String externalId, String texto, String distilledJson, int ord) {
    ...
    private void insertarChunkEnDocumento(
        ...
        jdbc.sql("""
                        ...
                        """)
                .param("documentoId", documentoId).param("fuenteId", fuenteId).param("ord", ord)
                .param("texto", texto).param("distilled", distilledJson)
                .update();
    }
}

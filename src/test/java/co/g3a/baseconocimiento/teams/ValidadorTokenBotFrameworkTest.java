...
/**
 * Gate de riesgo del plan (sección Pruebas automatizadas de F5): "validación
 * del JWT de Bot Connector con un JWKS de prueba, incluyendo tokens vencidos y
 * con audiencia equivocada". WireMock dobla tanto el documento OpenID como el
 * JWKS; los tokens se firman de verdad con una clave RSA generada en el test.
 */
class ValidadorTokenBotFrameworkTest {
    ...
    void generarClavesYPublicarJwks() throws Exception {
        ...
        claves = new RSAKey.Builder((RSAPublicKey) par.getPublic())
                .privateKey((RSAPrivateKey) par.getPrivate())
                .keyID("kid-de-prueba")
                .build();

        wireMock.stubFor(get(urlEqualTo("/jwks")).willReturn(aResponse()
                .withHeader("Content-Type", "application/json")
                .withBody(new JWKSet(claves.toPublicJWK()).toString())));
        wireMock.stubFor(get(urlEqualTo("/canal")).willReturn(aResponse()
                .withHeader("Content-Type", "application/json")
                .withBody("{\"jwks_uri\":\"" + wireMock.baseUrl() + "/jwks\"}")));
        wireMock.stubFor(get(urlEqualTo("/emulador")).willReturn(aResponse()
                .withHeader("Content-Type", "application/json")
                .withBody("{\"jwks_uri\":\"" + wireMock.baseUrl() + "/jwks\"}")));
    }
    ...
    private String tokenDeCanal(String audiencia, String serviceUrl, Instant expiracion) throws Exception {
    ...
    void aceptaUnTokenDeCanalValido() throws Exception {
        ...
        assertThatCode(() -> validador.validar("Bearer " + token, SERVICE_URL)).doesNotThrowAnyException();
    }
    ...
    void rechazaUnServiceUrlDistintoAlDeLaActivityEntrante() throws Exception {
        ...
        assertThatThrownBy(() -> validador.validar("Bearer " + token, "https://otro-service-url.example.com/"))
                .isInstanceOf(ValidadorTokenBotFramework.TokenInvalidoException.class);
    }
    ...
    void sinCredencialesConfiguradasNoValidaNada() {
        var propiedadesSinAppId = new TeamsPropiedades(
                true, false, "", "", wireMock.baseUrl() + "/canal", wireMock.baseUrl() + "/emulador",
                wireMock.baseUrl() + "/token", "scope");

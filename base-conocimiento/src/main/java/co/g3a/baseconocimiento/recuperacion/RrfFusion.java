...
/**
 * Fusión por rango (Reciprocal Rank Fusion), como en el artículo: {@code score(d) = Σ peso / (k +
 * rango)}, SIN normalizar los puntajes crudos de cada señal — sus escalas no son comparables entre
 * sí (un {@code ts_rank_cd} no es una similitud coseno), pero sus RANGOS sí lo son. Un chunk
 * ausente de una señal aporta 0 a esa señal, sin penalizarlo.
 *
 * <p>Deliberadamente sin estado y sin dependencias de Spring: los pesos, la {@code k} y los topes
 * los decide quien llama ({@code Recuperador}, que sí lee configuración), no esta clase. Así se
 * puede probar por propiedades sin levantar un contexto.
 */
final class RrfFusion {
  ...
  /**
   * Fusiona las listas de cada señal (ya ordenadas desc por puntaje) y devuelve candidatos diversos
   * ordenados por RRF desc: como mucho {@code topePorDocumento} por documento, y como mucho {@code
   * maxCandidatos} en total.
   */
  static List<CandidatoFusionado> fusionar(
    ...
    List<CandidatoFusionado> ordenados =
        acumuladores.values().stream()

 ...
 *
 * <p>Vive separado de {@link TrabajadorEmbebido} para que {@code @Transactional} en {@code
 * procesarUno()} funcione de verdad — ver el comentario en esa clase.
 *
 * <p>Se puede apagar con {@code kb.ingesta.worker.habilitado=false}: sirve para pruebas de
 * integración que no necesitan (ni pueden, si no hay Ollama) que la cola se procese sola de fondo.
 */
@Component
...
class TrabajadorEmbebidoProgramador {
  ...
  /**
   * Lanza el lote entero en paralelo sobre hilos virtuales: cada {@code procesarUno()} toma su
   * propio trabajo con {@code SKIP LOCKED}, así que no compiten entre sí por fila. Ya no corta al
   * primer "no hay más trabajo" (antes era secuencial y eso servía de atajo) — con 25 hilos
   * virtuales de por medio, casi todos devuelven falso de inmediato si la cola ya está vacía, así
   * que el costo de no cortar antes es despreciable.
   */

...
/**
 * Dispara la ingesta a mano. Vive dentro de {@code ingesta}, no en
 * {@code web}: es un endpoint de operación que el propio módulo expone sobre
 * sí mismo, no una puerta de usuario final — por eso la regla de ArchUnit que
 * aísla a los adaptadores no le aplica.
 *
 * <p>{@code make ingest} le apunta directo. Delega en {@link RelevadorDeFuentes}
 * en vez de llamar a cada conector directo (como hacía antes de F8): así una
 * llamada manual, el relevo periódico y el botón "reindexar ahora" de la
 * consola de F9 comparten el mismo candado por tipo, y no pueden pisarse.
 */
@RestController

...
/**
 * Valida el JWT que llega en el header {@code Authorization} de {@code POST /api/messages} contra
 * el JWKS de Bot Framework.
 *
 * <p>La lógica está portada de {@code ChannelValidation}/{@code EmulatorValidation} del Bot
 * Framework SDK para Java (archivado en enero de 2026, ver el plan): dos documentos OpenID
 * distintos según el modo, dos formas de leer el appId del token (audience directa para canal real;
 * {@code appid} o {@code azp} según la versión del token para el Emulator, porque Azure AD v1 y v2
 * lo exponen distinto), y la misma verificación final del claim {@code serviceurl} contra la
 * Activity recién recibida — sin esta última comprobación, un token robado de otra conversación
 * serviría igual aquí. Constantes verificadas contra el código fuente actual de {@code
 * AuthenticationConstants.java}, no contra memoria.
 */
@Component
class ValidadorTokenBotFramework {
  ...
  /**
   * Los 6 issuers que Azure AD emite para el Emulator: dos GUID de tenant (el tenant público {@code
   * botframework.com} y su equivalente de US Government), cada uno en v1 ({@code sts.windows.net})
   * y v2 ({@code login.microsoftonline.com}/{@code .us}).
   */
  static final Set<String> ISSUERS_EMULATOR =
      Set.of(
  ...
  private final ConcurrentHashMap<String, JwtDecoder> decodersPorMetadata =
  ...
  /**
   * No lanza nada si {@code kb.teams.app-id} está vacío: sin credenciales configuradas el producto
   * se demuestra completo igual (ver Supuestos del plan) y no hay contra qué credencial validar.
   */
  void validar(String encabezadoAutorizacion, String serviceUrlEntrante) {
    ...
    if (encabezadoAutorizacion == null
        || !encabezadoAutorizacion.regionMatches(true, 0, "Bearer ", 0, 7)) {
    ...
    if (serviceUrlDelToken == null || !serviceUrlDelToken.equalsIgnoreCase(serviceUrlEntrante)) {
      throw new TokenInvalidoException(
          "El claim serviceurl del token no coincide con la Activity entrante");
    }
  }
    ...
    if (propiedades.emulator()) {
      ...
      String appIdDelToken =
          VERSION_2.equals(version)
              ? jwt.getClaimAsString(CLAIM_AZP)
              : jwt.getClaimAsString(CLAIM_APPID);
  ...
  private JwtDecoder decoder() {
    String metadataUrl =
        propiedades.emulator()
            ? propiedades.emulatorMetadataUrl()
            : propiedades.channelMetadataUrl();
  ...
  private JwtDecoder construirDecoder(String metadataUrl) {
    ...
    log.info(
        "Decoder de JWT de Bot Framework construido contra {} (jwks_uri={})", metadataUrl, jwksUri);
    return decoder;
  }
  ...
  private String resolverJwksUri(String metadataUrl) {
    ...
    if (nodo == null || nodo.isNull() || nodo.isMissingNode()) {
      throw new IllegalStateException(
          "El documento OpenID de " + metadataUrl + " no trae jwks_uri");
    }

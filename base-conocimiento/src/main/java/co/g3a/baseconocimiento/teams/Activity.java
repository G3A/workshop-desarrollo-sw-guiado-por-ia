...
/**
 * Subconjunto del esquema {@code Activity} del protocolo Bot Connector: solo los campos que este
 * adaptador necesita para leer un mensaje entrante y armar la respuesta.
 * {@code @JsonIgnoreProperties(ignoreUnknown = true)} porque una Activity real trae muchos más
 * campos (entities, channelData, locale...) que no nos hace falta modelar.
 */
...
public record Activity(
  ...
  /**
   * La Activity de respuesta: intercambia {@code from}/{@code recipient} y referencia el mensaje
   * original.
   */
  static Activity respuestaA(
      Activity entrante, String tipo, List<Attachment> attachments, String texto) {
    return new Activity(
        tipo,
        null,
        null,

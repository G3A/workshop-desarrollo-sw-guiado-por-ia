...
/**
 * Responde una Activity de tipo {@code message}: indicador de escritura, las siete etapas del
 * pipeline vía {@link Consultar}, y la respuesta como Adaptive Card.
 *
 * <p>Separado de {@link BotController} a propósito: {@link #procesar} corre sincrónico y se puede
 * probar directo, sin depender del hilo virtual que {@link #procesarAsync} usa de verdad contra una
 * petición HTTP real.
 */
@Component
class ProcesadorDeMensajes {
  ...
  /**
   * El webhook de {@code /api/messages} ya devolvió 200: esto corre en su propio hilo virtual
   * porque la síntesis tarda minutos en CPU (ver Riesgos vivos del plan) y no puede bloquear esa
   * respuesta.
   */
  ...
  void procesar(Activity actividad) {
    ...
    try {
      Respuesta respuesta =
          consultar.responder(
              new Pregunta(actividad.text()), ProyectoId.POR_DEFECTO, Filtros.NINGUNO);
      conector.responderConTarjeta(actividad, respuesta);
    } catch (Exception e) {
      log.error("Fallo respondiendo la pregunta de Teams: {}", e.toString());
      conector.responderTexto(
          actividad, "Hubo un error respondiendo la pregunta. Intenta de nuevo.");
    }
  }
}

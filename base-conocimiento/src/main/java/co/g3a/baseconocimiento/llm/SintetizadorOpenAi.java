...
/**
 * Síntesis en streaming. El prompt de sistema es la única defensa contra la alucinación: responder
 * SOLO con lo que trae el contexto, citar cada afirmación con {@code [n]}, y señalar una
 * contradicción en vez de elegir un lado en silencio cuando dos fuentes no coinciden.
 *
 * <p>Es el {@link Sintetizador} activo por defecto ({@code
 * kb.llm.sintesis-estructurada.habilitada:false}). La alternativa, {@link
 * SintetizadorEstructuradoOpenAi}, fuerza salida JSON en vez de prosa libre — ver su javadoc para
 * cuándo conviene probarla.
 *
 * <p>{@code extraBody(repeat_penalty)}: sin este parámetro, la sesión 6 de la investigación
 * (ADR-0009) midió que este mismo binario (Bonsai 8B via llama-server) repetía la respuesta
 * completa dos veces con el sampling por defecto — un modo de falla que no aparecía con {@code
 * gemma3:4b}/Ollama. No es parte de la API oficial de OpenAI, por eso va en {@code extraBody}. Ya
 * corregido eso, la citacion todavia midio peor que en las pruebas aisladas de la sesion 5
 * (fragmento irrelevante citado, marcador antes de la afirmacion en vez de despues) — sigue
 * pendiente mas ajuste y re-validacion, no es un problema resuelto del todo.
 *

...
/**
 * Alternativa a {@link SintetizadorOpenAi}: en vez de prosa libre con marcadores {@code [n]}
 * incrustados a mano, fuerza la síntesis como salida JSON estructurada (una lista de afirmaciones,
 * cada una con su fuente), igual que ya hacen {@link PlanificadorOpenAi} y {@link
 * VerificadorGroundingOpenAi} — y ese mismo patrón sí les funcionó a modelos que, en texto libre,
 * pegaban el fragmento crudo en vez de redactar (`granite4.1:3b`, `phi4-mini:3.8b`, `qwen2.5:3b`,
 * `nemotron-mini:4b`, `SmolLM3-3B` — ver sesiones 2-23 de docs/investigacion-vram-y-modelo-llm.md).
 * Es la vía que la sesión 3 de esa investigación había dejado anotada como no explorada.
 *
 * <p><b>Desactivado por defecto</b> ({@code kb.llm.sintesis-estructurada.habilitada:false}): {@link
 * SintetizadorOpenAi} sigue siendo el {@link Sintetizador} real de producción sin ningún cambio de
 * comportamiento. Activar esta clase es responsabilidad explícita de quien la prueba ({@code
 * KB_LLM_SINTESIS_ESTRUCTURADA_HABILITADA=true}), no un default nuevo.
 *
 * <p><b>Trade-off sin resolver, documentado a propósito</b>: {@link Sintetizador} devuelve {@code
 * Flux<String>} porque {@link SintetizadorOpenAi} transmite token a token para la UI en streaming
 * (ver {@code Orquestador#ejecutarEnStreaming}). La salida JSON forzada no se puede transmitir así
 * de forma incremental sin parsear JSON parcial — esta implementación arma la respuesta completa en
 * una sola llamada bloqueante y la devuelve como un {@link Flux} de un solo elemento. Sigue
 * cumpliendo el contrato de la interfaz sin tocar {@code Orquestador} ni el adaptador SSE, pero la
 * UI verá la respuesta aparecer de una sola vez en vez de palabra por palabra. Nadie midió todavía
 * si esto es aceptable para el caso de uso real — parte de por qué esta clase arranca apagada.
 *
 * <p>Cada {@code Afirmacion} lleva UNA sola fuente ({@code int}, no una lista): a diferencia del
 * prompt de texto libre, que le pedía al modelo "un solo marcador por afirmación" y aun así
 * `gemma3:4b` agrupaba varios (sobre-citación, hallazgo 6 de la investigación), acá el esquema lo
 * impide por construcción — si una afirmación necesita dos fuentes, el modelo tiene que partirla en
 * dos afirmaciones, no hay campo para poner dos números.
 */
@Component
@ConditionalOnProperty(
    prefix = "kb.llm.sintesis-estructurada",
    name = "habilitada",
    havingValue = "true")
class SintetizadorEstructuradoOpenAi implements Sintetizador {
  ...
  /**
   * @param fuente el número [n] del fragmento del contexto que respalda a {@code texto}, tal como
   *     aparece en el prompt (1-indexado)
   */

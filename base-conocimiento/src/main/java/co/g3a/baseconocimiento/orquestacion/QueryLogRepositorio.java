...
/**
 * Etapa 7: el registro de auditoría. Sin esto no se depuran respuestas malas a
 * escala — el artículo lo señala explícitamente, y es la razón de que
 * {@code query_log} exista desde el esquema de F0.
 */
@Repository
class QueryLogRepositorio {
    ...
    long registrar(
            String pregunta, String projectId, PlanDeHerramientas plan,
            List<Executor.EjecucionHerramienta> herramientas, List<Fragmento> fragmentosUsados,
            String respuesta, List<Cita> citas, long latenciaMs) {

        List<Map<String, Object>> toolsRun = herramientas.stream()
                .map(h -> Map.<String, Object>of(
                        "nombre", h.nombre(), "fragmentos", h.fragmentos().size(),
                        "duracionMs", h.duracionMs(), "error", h.error() == null ? "" : h.error()))
                .toList();

        List<Map<String, Object>> candidatos = fragmentosUsados.stream()
                .map(f -> Map.<String, Object>of(
                        "id", f.id(), "uri", f.uri(), "titulo", f.titulo() == null ? "" : f.titulo(),
                        "rrf", f.rrf(), "rerank", f.rerank() == null ? -1.0 : f.rerank()))
                .toList();

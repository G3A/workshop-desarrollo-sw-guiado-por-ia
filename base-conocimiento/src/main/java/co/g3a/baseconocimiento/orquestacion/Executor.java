...
/**
 * Etapas 2 y 3 del pipeline: toma la lista de herramientas que eligió el planner y las corre en
 * paralelo, sobre hilos virtuales — igual que {@code Recuperador} hace con las cuatro señales en
 * F2.
 *
 * <p>Aísla fallos por herramienta: si una revienta (Postgres momentáneamente caído, {@code rg} no
 * instalado), las demás siguen y esa queda registrada con su error en la traza en vez de tumbar
 * toda la pregunta.
 */
@Component
class Executor {
  ...
  /**
   * @param error {@code null} si la herramienta terminó bien, aunque haya devuelto una lista vacía
   */
  record EjecucionHerramienta(
      String nombre, List<Fragmento> fragmentos, long duracionMs, String error) {}

  List<EjecucionHerramienta> ejecutar(
      List<String> nombresHerramientas,
      String consulta,
      ProyectoId proyecto,
    ...
    try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
      List<Future<EjecucionHerramienta>> futuros =
          seleccionadas.stream()
              .map(
                  h ->
                      executor.submit(
                          () -> ejecutarUna(h, consulta, proyecto, documentosPermitidos)))
              .toList();
  ...
  /**
   * Si el planner no eligió nada resoluble (lista vacía, o nombres que no existen en el catálogo),
   * cae a {@code search_unified}: la pregunta nunca se queda sin ninguna búsqueda por un plan
   * imperfecto.
   */
  private List<Herramienta> resolver(List<String> nombresHerramientas) {
    List<Herramienta> resueltas =
        nombresHerramientas.stream().map(catalogo::porNombre).flatMap(Optional::stream).toList();
  ...
  private static EjecucionHerramienta ejecutarUna(
    ...
    try {
      ...
      return new EjecucionHerramienta(
          herramienta.nombre(), List.of(), duracionMs(inicio), e.toString());
    }
  }

 ...
 *
 * <p>Modelo y tokenizador se cargan de forma perezosa en el primer
 * {@link #puntuar(String, String)}, no al arrancar la aplicacion: cargar ~550 MB
 * antes de que exista una sola consulta haria que un `docker compose up` sin
 * modelos descargados nunca llegara a estar sano. {@code make pull-models} es
 * quien los deja en {@code kb.reranker.ruta}.
 *
 * <p>El formato del par consulta/pasaje y el post-procesamiento (sigmoid sobre
 * el logit crudo) siguen el uso documentado de BAAI/bge-reranker-v2-m3: un solo
 * llamado al tokenizador con (consulta, pasaje) como par, y
 * {@code sigmoid(logits[0])} como puntaje de relevancia en [0,1]. Aqui se
 * multiplica por 10 para la escala 0-10 que el resto del pipeline espera.
 */
@Component
class RerankerOnnx implements Reranker, AutoCloseable {
    ...
    public double puntuar(String consulta, String pasaje) {
        ...
        try {
            ...
            entradas.put("attention_mask", OnnxTensor.createTensor(entorno, LongBuffer.wrap(mascara), forma));
            ...
            if (nombresEsperados.contains("token_type_ids")) {
                entradas.put("token_type_ids", OnnxTensor.createTensor(entorno, LongBuffer.wrap(tipos), forma));
            }
    ...
    private void cargar() {
        ...
        try {
            ...
            this.tokenizador = HuggingFaceTokenizer.builder()

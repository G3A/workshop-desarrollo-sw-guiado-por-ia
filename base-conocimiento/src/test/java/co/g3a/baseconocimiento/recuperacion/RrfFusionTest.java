...
/**
 * Propiedades de {@link RrfFusion}: la fusión por rango del artículo, sin
 * normalizar puntajes. Ver la sección "Pruebas automatizadas" del plan.
 */
class RrfFusionTest {
    ...
    @Property
    @DisplayName("Dentro de una sola senal, la fusion respeta el rango: el puntaje decrece estrictamente")
    ...
    void elConsensoGanaAUnPrimerPuestoAislado(
        ...
        assertThat(rrfPorId.get(2L))
                .as("el consenso (nunca primero, pero presente en varias senales) debe superar "
    ...
    void topePorDocumentoLimitaDiversidad() {
        List<CandidatoSenal> tresDelMismoDocumento = List.of(
        ...
        List<CandidatoFusionado> fusionados = RrfFusion.fusionar(
                Map.of(Senal.FTS, tresDelMismoDocumento), PESOS_IGUALES, 60, 2, 10);
    ...
    private static CandidatoSenal candidatoEnDocumento(long id, long documentoId, double puntaje) {
        return new CandidatoSenal(id, documentoId, "file:///" + id, "titulo " + id, "texto " + id,
                "doc_section", 0, Instant.EPOCH, puntaje);
    }
}

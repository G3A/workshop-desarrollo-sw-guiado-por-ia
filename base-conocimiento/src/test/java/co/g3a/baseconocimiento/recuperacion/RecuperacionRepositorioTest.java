...
/**
 * Las cuatro señales contra un PostgreSQL real, más el recálculo de
 * {@code term_stats} que alimenta la señal 3. No prueba el reranker (eso pide
 * el ONNX real descargado por {@code make pull-models}, ver
 * {@code RerankerOnnxTest}) — solo el SQL de {@link RecuperacionRepositorio}.
 */
@SpringBootTest(
...
class RecuperacionRepositorioTest {
    ...
    @Autowired
    RecuperacionRepositorio repositorio;

    @Test
    @DisplayName("Senal 1 (texto completo): encuentra por la pregunta destilada")
    void senalFts() {
        long id1 = insertarChunk("fts-1", "texto crudo irrelevante",
                "{\"searchable_question\":\"como se despliega el servicio\"}", null);
    ...
    void filtroDeTipos() {
        long doc = insertarChunk("tipo-doc", "contenido de documentacion sobre despliegue",
                "{\"summary\":\"contenido de documentacion sobre despliegue\"}", null);
        long codigo = insertarChunkConTipo("tipo-code", "contenido de documentacion sobre despliegue",
    ...
    @Test
    @DisplayName("Senal 3 (IDF): tras recalcular term_stats, un termino raro pesa mas que uno de relleno")
    void senalIdf() {
        // "desplegar" aparece en un solo chunk (informativo, IDF alto).
        // "sistema" aparece en los otros tres (relleno, IDF bajo).
        long informativo = insertarChunk("idf-1", "", "{\"summary\":\"para desplegar el servicio corre el script\"}", null);
        insertarChunk("idf-2", "", "{\"summary\":\"el sistema registra cada evento del sistema\"}", null);
        insertarChunk("idf-3", "", "{\"summary\":\"el sistema arranca junto con el sistema operativo\"}", null);
    ...
    private long insertarChunk(String externalId, String texto, String destiladoJson, float[] embedding) {
        return insertarChunk(externalId, texto, destiladoJson, embedding, "doc_section");
    }

    private long insertarChunkConTipo(String externalId, String texto, String destiladoJson, String tipo) {
    ...
    private long insertarChunk(
        ...
        long chunkId = jdbc.sql("""
                        ...
                        RETURNING id
                        """)
                .param("e", externalId).param("t", texto).param("j", destiladoJson).param("tipo", tipo)
                .query(Long.class).single();

        if (embedding != null) {
            jdbc.sql("UPDATE chunks SET embedding = :embedding WHERE id = :id")
                    .param("embedding", new PGvector(embedding)).param("id", chunkId).update();
        }
        return chunkId;
    }
}

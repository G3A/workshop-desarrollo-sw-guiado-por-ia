...
/**
 * {@link ClienteConectorBotFramework} contra un Bot Connector API doblado con WireMock: verifica el
 * POST de {@code ReplyToActivity} para el indicador de escritura y para la tarjeta, y que el token
 * de salida se pida una sola vez por client_credentials y se reutilice mientras esté vigente.
 */
class ClienteConectorBotFrameworkTest {
  ...
  private static final Activity ACTIVIDAD_BASE =
      new Activity(
          "message",
          "activity1",
          null,
  ...
  private Activity actividadEntrante() {
    return new Activity(
        ...
        null,
        wireMock.baseUrl(),
        ACTIVIDAD_BASE.channelId(),
        ACTIVIDAD_BASE.from(),
        ACTIVIDAD_BASE.conversation(),
        ACTIVIDAD_BASE.recipient(),
        ACTIVIDAD_BASE.text(),
        null,
        null);
  }
  ...
  void enviaElIndicadorDeEscrituraSinAutorizacionCuandoNoHayCredenciales() {
    wireMock.stubFor(
        post(urlEqualTo("/v3/conversations/conv1/activities/activity1"))
    ...
    wireMock.verify(
        postRequestedFor(urlEqualTo("/v3/conversations/conv1/activities/activity1"))
  ...
  void responderConTarjetaEnviaElAdaptiveCardComoAttachment() {
    ...
    Respuesta respuesta =
        new Respuesta(
            "la respuesta",
            List.of(new Cita("file:///doc1", "Doc 1", "extracto", "doc_section")),
            List.of(),
            10,
            null);

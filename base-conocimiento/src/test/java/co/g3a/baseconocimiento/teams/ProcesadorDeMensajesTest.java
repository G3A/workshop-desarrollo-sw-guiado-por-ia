...
/**
 * {@link ProcesadorDeMensajes#procesar} corre sincrónico a propósito (ver su Javadoc): esta prueba
 * lo llama directo, sin pasar por el hilo virtual real de {@code procesarAsync}, ni por HTTP, ni
 * por Postgres/Ollama.
 */
class ProcesadorDeMensajesTest {

  private static final Activity ACTIVIDAD =
      new Activity(
          "message",
          "activity1",
          null,
          "https://smba.example.com/",
          "test",
  ...
  void enElCaminoFelizEnviaEscribiendoYLuegoLaTarjeta() {
    ...
    Respuesta respuesta =
        new Respuesta(
            ...
            List.of(new Cita("file:///readme", "Readme", "extracto", "doc")),
            List.of(),
            42,
            null);
    ...
    verify(consultar)
        .responder(
            new Pregunta("como se despliega el servicio?"),
  ...
  void siConsultarFallaEnviaUnaRespuestaDeTextoDeFallback() {
    Consultar consultar = mock(Consultar.class);
    ClienteConectorBotFramework conector = mock(ClienteConectorBotFramework.class);
    when(consultar.responder(any(), any(), any()))

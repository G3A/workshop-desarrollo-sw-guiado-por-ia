 ...
 * autoconfiguraciones se partieron en modulos, y tener {@code flyway-core} en el classpath ya no
 * basta para que Flyway corra. La aplicacion arrancaba contra una base vacia, reportaba salud UP y
 * no decia nada. Lo unico que evita que eso vuelva a pasar es una asercion que lo compruebe en cada
 * build.
 */
@SpringBootTest(
...
class EsquemaTest {
  ...
  void flywayCorrio() {
    ...
    assertThat(aplicadas)
        .as(
            "si esto falla, revisa que `spring-boot-flyway` siga en el pom: "
  ...
  void tablasCreadas() {
    ...
    assertThat(tablas)
        .contains("sources", "documents", "chunks", "term_stats", "ingest_jobs", "query_log");
  }
  ...
  void loDestiladoDominaAlCrudo() {
    ...
    assertThat(porDestilado).as("la pregunta destilada debe encontrarse").isNotNull();
    assertThat(porCrudo)

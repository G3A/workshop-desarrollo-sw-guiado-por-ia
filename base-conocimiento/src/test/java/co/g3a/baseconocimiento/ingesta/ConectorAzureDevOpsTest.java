...
/**
 * {@link ConectorAzureDevOps} contra la REST API de Azure DevOps doblada con
 * WireMock: WIQL + work items batch, y el árbol completo de páginas de wiki
 * con su contenido, croceado por {@code ChunkerEncabezados}.
 */
@SpringBootTest(
...
class ConectorAzureDevOpsTest {
    ...
    void ingiereWorkItemsYWiki() {
        wireMock.stubFor(post(urlPathEqualTo("/myorg/myproj/_apis/wit/wiql"))
                .willReturn(aResponse().withHeader("Content-Type", "application/json").withBody("""
                        {"workItems": [{"id": 1, "url": "https://dev.azure.com/myorg/_apis/wit/workItems/1"}]}
                        """)));

        wireMock.stubFor(post(urlPathEqualTo("/myorg/_apis/wit/workitemsbatch"))
                .willReturn(aResponse().withHeader("Content-Type", "application/json").withBody("""
                        {
                          "count": 1,
                          "value": [
        ...
        wireMock.stubFor(get(urlPathEqualTo("/myorg/myproj/_apis/wiki/wikis/mywiki/pages"))
        ...
        List<String> textosWorkItem = jdbc.sql("""
        ...
        List<String> tiposWiki = jdbc.sql("""

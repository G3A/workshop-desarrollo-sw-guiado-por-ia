...
/**
 * {@link AdminController} contra un slice de MVC, con {@link IngestaRepositorio} y {@link
 * RelevadorDeFuentes} doblados. La carga de archivos habilitada tiene su propia clase ({@link
 * AdminControllerCargaHabilitadaTest}): necesita otra propiedad y un directorio real.
 */
...
class AdminControllerTest {
  ...
  void fuentesJuntaFuenteYUltimoResultado() throws Exception {
    var fuente =
        new FuenteAdmin(1L, "local_docs", "corpus", "default", true, Instant.EPOCH, 900, 3, 10);
  ...
  void reindexarDelegaEnRelevador() throws Exception {
    when(relevador.relevar("local_docs"))
        .thenReturn(new ResultadoRelevo("local_docs", true, "ok", null));

    mockMvc
        .perform(post("/api/admin/fuentes/local_docs/reindexar"))
        .andExpect(status().isOk())
  ...
  void proyectosDevuelveLosProjectIdReales() throws Exception {
    ...
    mockMvc
        .perform(get("/api/admin/proyectos"))
        .andExpect(status().isOk())
  ...
  @Test
  @DisplayName(
      "POST /api/admin/vault/documentos rechaza con 403 cuando la carga esta deshabilitada")
  ...
  @Test
  @DisplayName(
      "GET /api/admin/vault/archivos deriva el estado efectivo a partir de los conteos de chunks")
  void archivosVaultDerivaElEstadoEfectivo() throws Exception {
    var enError =
        new IngestaRepositorio.ArchivoVaultAdmin(
            1L,
            10L,
            "local_docs",
            "documentos",
    ...
    mockMvc
        .perform(get("/api/admin/vault/archivos"))
        .andExpect(status().isOk())

...
/**
 * {@link ConectorReposLocales} contra un repo Git real (JGit lo crea en un
 * directorio temporal) y un PostgreSQL real vía Testcontainers — el criterio
 * de salida de F6 pide chunks consultables, no un doble de la ingesta.
 */
@SpringBootTest(
...
class ConectorReposLocalesTest {
    ...
    void ingiereUnRepoNuevo() throws Exception {
        crearRepoConUnArchivo("mi-repo", "Saludo.java", """
                public class Saludo {
                    void saludar() {
    ...
    private void crearRepoConUnArchivo(String nombreRepo, String archivo, String contenido) throws Exception {
        ...
        try (Git git = Git.init().setDirectory(repoPath.toFile()).call()) {
            ...
            git.commit().setMessage("inicial").setAuthor("Test", "test@example.com").setSign(false).call();
        }
    }

...
/**
 * El fan-out y el aislamiento de fallos del {@link Executor}, sin Spring ni base de datos:
 * herramientas de mentira que se comportan bien o mal a propósito.
 */
class ExecutorTest {
  ...
  void aislaElFalloDeUnaHerramienta() {
    var catalogo =
        new CatalogoHerramientas(
            List.of(herramientaQueDevuelve("buena", fragmento(1)), herramientaQueFalla("mala")));
  ...
  void caeASearchUnifiedSiElPlanNoResuelveNada() {
    var catalogo =
        new CatalogoHerramientas(List.of(herramientaQueDevuelve("search_unified", fragmento(9))));
    var executor = new Executor(catalogo);
  ...
  private static Executor.EjecucionHerramienta porNombre(
      List<Executor.EjecucionHerramienta> lista, String nombre) {

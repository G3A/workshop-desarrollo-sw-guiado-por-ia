...
while IFS= read -r sub; do
  ...
  case "$CMDWORD" in
    Start-Process|start|saps)
      ...
      for a in "$@"; do
        case "$a" in
          -Verb:RunAs|-verb:runas|-Verb:Runas) hook_deny 'Blocked: Start-Process -Verb RunAs requests elevation. A change that needs elevation is a change for the user to make deliberately, not for the agent to run.' ;;
        esac
        case "$PREV" in
          -Verb|-verb) case "$a" in RunAs|runas|Runas) hook_deny 'Blocked: Start-Process -Verb RunAs requests elevation. A change that needs elevation is a change for the user to make deliberately, not for the agent to run.' ;; esac ;;
        esac
        PREV="$a"
      done
      ;;
  ...
  case "$CMDWORD" in
    Remove-Item|ri|rm|rd|del|erase)
      ...
      for a in "$@"; do
        case "$a" in
          -Recurse|-recurse|-Recurse:*|-recurse:*) RECURSIVE=yes ;;
          -Path:*|-path:*|-LiteralPath:*|-literalpath:*) check_target "${a#*:}" ;;
